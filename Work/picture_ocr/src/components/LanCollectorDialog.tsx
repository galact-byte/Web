...
interface LanCollectorDialogProps {
  open: boolean;
  snapshot: LanCollectorSnapshot | null;
...
const LanCollectorDialog: React.FC<LanCollectorDialogProps> = ({ open, snapshot, bridge, sessionNotice, onClose, onSessionStatusChange }) => {
  ...
  const handleStart = async () => {
    ...
    if (!snapshot || snapshot.systems.length === 0) {
      setMessage('项目结构尚未就绪，请稍候或先在电脑端建好系统。');
      return;
    }
    setBusy(true);
    setMessage('');
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="lan-collector-title" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
      <section className="w-full max-w-lg border border-slate-300 bg-white p-6 shadow-lg">
        <div className="flex items-start justify-between gap-4"><div><h2 id="lan-collector-title" className="text-xl font-bold text-slate-950">手机局域网采集</h2><p className="mt-1 text-sm text-slate-600">手机可在采集页选择本项目组内任意系统，图片直接同步到电脑对应系统。</p></div><button type="button" onClick={onClose} disabled={busy} className="min-h-11 min-w-11 border border-slate-300 text-lg text-slate-700 hover:bg-slate-100" aria-label="收起手机局域网采集对话框">×</button></div>
