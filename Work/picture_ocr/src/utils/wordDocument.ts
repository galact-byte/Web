...
const getNoBorder = () => ({ style: docx.BorderStyle.NONE, size: 0, color: 'FFFFFF' });
const getSingleBorder = () => ({ style: docx.BorderStyle.SINGLE, size: 4, color: 'auto', space: 0 });
...
function templateText(text: string, size = 32, font = '仿宋', bold = false, color = '000000'): TextRun {
  return new docx.TextRun({ text, bold, size, font, color });
}
...
async function buildCoverPage(meta: ProjectMeta): Promise<Paragraph[]> {
  ...
  try {
    ...
    decorationImage = new docx.ImageRun({
      data: buffer,
  ...
  result.push(
    new docx.Paragraph({
      alignment: docx.AlignmentType.CENTER,
      spacing: { after: 200 },
      children: [new docx.TextRun({ text: '(V1.0)', bold: true, size: 36, font: '黑体', color: '000000' })],
    })
  );

  // 让 Logo 独立位于页面中段，不与标题或单位信息挤在一起。
  result.push(new docx.Paragraph({ spacing: { before: 1800 } }));
  ...
  result.push(new docx.Paragraph({ spacing: { before: 3800 } }));
  ...
  result.push(
    new docx.Paragraph({
      alignment: docx.AlignmentType.CENTER,
      children: [new docx.TextRun({ text: formatDateChinese(meta.reportDate), size: 44, font: '黑体', color: '000000' })],
    })
  );
...
async function buildAssetItemsTable(items: { label: string; images: ImageData[] }[]): Promise<Table> {
  ...
  return new docx.Table({
    ...
    borders: {
      top: getSingleBorder(),
      bottom: getSingleBorder(),
      left: getSingleBorder(),
      right: getSingleBorder(),
      insideHorizontal: getSingleBorder(),
      insideVertical: getSingleBorder(),
    },
    rows,
  });
}
...
function noTableBorders() {
  return {
    ...
    left: getNoBorder(),
    right: getNoBorder(),
    insideHorizontal: getNoBorder(),
    insideVertical: getNoBorder(),
  };
}
